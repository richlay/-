...
GetBmi <- function (my.height.cm, my.weight.kg){
  ...
  mybmi = my.weight.kg/(my.height.m^2)
  return(mybmi)
}
...
df <- read.table('test_data.csv', sep =',', header = F, stringsAsFactors = F)
...
df$Age <- as.Age(df$Age)
df$Height <- as.Height(df$Height)
df$Weight <- as.Weight(df$Weight)
